...
# GLOBAL STORAGE for the timing/efficiency plot
timing_results = {name: {label: np.zeros(len(query_counts)) for label, _, _, _ in methods} for name, _ in datasets}
trial_errors = {name: {label: np.zeros((len(query_counts), trials)) for label, _, _, _ in methods} for name, _ in datasets}
    ...
    for label, func, color, fmt in methods:
        ...
        for i, q in enumerate(query_counts):
            for t in range(trials):
                ...
                timing_results[name][label][i] += elapsed
                errors[i, t] = np.abs(est - true_tr) / true_tr

        trial_errors[name][label] = errors.copy()
...
for ax, (name, _) in zip(axes_flat, datasets):
    for label, _, color, fmt in methods:
        mu_error = np.mean(trial_errors[name][label], axis=1)
        # Average time across trials for THIS specific dataset
        avg_time = timing_results[name][label] / trials
        
        # Plot Time on X-axis and Error on Y-axis
