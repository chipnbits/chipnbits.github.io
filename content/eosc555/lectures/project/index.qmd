...
description: >-
    Geomagnetic inversion is a common problem in geophysics where the goal is to recover a 3D distribution of physical properties of a section of Earth using magnetic field measurements taken over a 2D surface above ground. The problem is ill-posed and requires advanced techniques to recover a model that matches the true distribution. In this project, a series of advanced techniques are applied to attempt to recover the true distribution.
categories:
  - Machine Learning
  - Neural Networks
  - Geophysics
...
Geomagnetic inversion from magnetic readings recovered above ground is a classic problem in geophysics. Much of the data that we have about subsurface geology must be collected via indirect methods such as magnetic and seismic surveys, or by sparse sampling via boreholes. The data can be expensive to collect, and the objective is typically to detect underground structures such as ore bodies, oil and gas deposits, or aquifers. The goal is to recover a 3D distribution of some physical property of the Earth from the incomplete data — a non-linear and ill-posed problem.

This project explores the use of machine learning and optimization techniques to attempt an inversion of the data, using electromagnetic equations to model the response of the subsurface to the Earth's magnetic field on a known dataset. The goal is to develop a method that can take the forward problem data and invert it to recover the true starting distribution.
...
The subsurface of the Earth is composed of materials with differences in magnetic susceptibility $\chi$ and conductivity $\sigma$. The magnetic susceptibility relates a material's magnetization $M$ to the magnetic field $H$ via the equation $M = \chi H$. When the magnetic field of the Earth passes through this material, it creates a magnetization response, causing small magnetic anomalies that can be measured above ground. The magnetic field passing through the subsurface is assumed to be uniform and known over the survey area.

The magnetization effect can be calculated by integrating the magnetic contribution over each infinitesimally small volume of the subsurface, since the total effect is the sum of the parts — similar to analyzing electric fields from charge distributions. It is easier to first calculate the magnetic vector potential and then take the curl to get the field:

$$\mathbf{B}(r) = \nabla \times \mathbf{A}(r) $$
$$\mathbf{A}(r) = \frac{\mu_0}{4\pi} \int_V \mathbf{M}(\mathbf{r'}) \times \frac{\mathbf{r} - \mathbf{r'}}{|\mathbf{r} - \mathbf{r'}|^3} \, dV'$$

where $\mathbf{B}$ is the magnetic field, $\mathbf{A}$ is the magnetic vector potential, $\mathbf{M}$ is the magnetization, and $\mu_0$ is the magnetic permeability of free space. The equations above are for the case where there are no free currents, which is a good assumption for the Earth's subsurface.

The integral has the form of a convolution with a kernel derived from $\frac{1}{|\mathbf{r} - \mathbf{r'}|}$, which can be computed using the Fast Fourier Transform (FFT) to speed up the computation. The operation $\mathbf{B}(r) = \nabla \times \mathbf{A}(r)$ can also be carried through into the integral, since it is a linear operator.

Using all of the above information yields an integral equation that depends on the magnetic susceptibility $\chi$ and the inducing field $\mathbf{H}_0$:

$$ \mathbf{B}_A(\mathbf{r}) = \frac{\mu_0}{4\pi} \int_V \chi(\mathbf{r'}) \, \nabla \nabla \left( \frac{1}{|\mathbf{r} - \mathbf{r'}|} \right) \mathbf{H}_0 \, dV' $$

where $\nabla \nabla$ denotes the second-derivative (Hessian) tensor of the kernel, not the Laplacian.
...
A simple toy dataset was used to test the adjoint operation. It is very helpful to have a set of visualization tools to verify the input and output data against expected values. A library of plotting functions that make use of [PyVista](https://docs.pyvista.org/) is provided below.
...
The magnetic anomaly readings above ground are but a 2D slice of the 3D data that is being recovered. This is like trying to reconstruct a 3D object from the shadow that it casts — a very ill-conditioned problem. To gauge the difficulty of the problem, we attempt a simple inversion using the conjugate gradient method developed in earlier lectures.

The inversion problem is to recover the magnetization model $M$ from the observed magnetic data $D$ using the forward model $A$, its adjoint $A^*$, and the conjugate gradient method. The null space of the forward operator is quite large, meaning there are infinitely many possible solutions, so a regularization term is added to the objective function to constrain the solution. The model is
...
where $R(M)$ is the regularization term and $\alpha$ is the regularization parameter. In this example, a weight matrix $W$ is applied to the flattened model.
...
The conjugate gradient method is used to solve the system of equations. The adjoint of the regularization operator is also defined, to speed up the inversion when using techniques that make use of it.

#### Regularization Implementation

The penalty, or prior, imposed here is the finite difference between neighboring cells in the model. This approximates the gradient at each point of the model, functioning as a slope penalty regularization as discussed previously in [Lecture 8](../lecture8/index.qmd#choice-of-regularizer).
...
and it can be extended to 2D and 3D by applying the finite difference along each dimension of the flattened data; it is therefore a linear operator. In practice, it is simpler to compute the value as a summation over the model cells: $$\|W(M)\|^2 = \sum_{i}\sum_{j}\sum_{k} (M_{i,j,k} - M_{i+1,j,k})^2 + (M_{i,j,k} - M_{i,j+1,k})^2 + (M_{i,j,k} - M_{i,j,k+1})^2$$ We take the mean over each dimension to normalize across any small variations in the data size due to the loss of boundary cells when applying the finite difference.
...
The inversion is clearly a failure. A shallow distribution of material with lower susceptibility can produce a 2D magnetic field essentially identical to that of the true model, in which the material is buried much deeper. The inversion has a preference for shallow material, as material that is far away contributes more weakly to the data — making it more susceptible to noise and closer to the null space of the forward operator.

To recover the true magnetic model, a more sophisticated inversion method is required. One such method is described in a review paper by Crespo Marques et al. [@CrespoMarques2019] and is a topic of exploration in the next project section, where a sparse recovery technique will be used to recover the true model.
