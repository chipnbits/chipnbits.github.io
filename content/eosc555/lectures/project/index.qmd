...
A generalized linear adjoint computation in PyTorch is also defined in code, so that the regularization term has a defined adjoint operation for the optimization algorithm.
...
def regularization(x):
    ...
    gy = x[:,:,:,1:,:] - x[:,:,:,:-1,:]
    gz = x[:,:,:,:,1:] - x[:,:,:,:,:-1]
    return 0.5*(torch.mean(gx**2)+torch.mean(gy**2)+torch.mean(gz**2))
...
def A(x, alpha=reg_param):
    ...
    y2 = regularization(x)
    y2 = adjoint(regularization, y2, x_sample = x)
    return y1 + alpha*y2
...
loss = 0.5 * (D - forMod(xtest)).norm()**2 + 0.5 * reg_param * regularization(xtest)**2
loss.backward()
...
reg = 0.5 * reg_param * regularization(xinv)**2
...
p = plot_model_with_forward(xinv, D, h, height=0, zoom = 4.0, threshold_value=0.01, plotter=p)
p.show()
```
