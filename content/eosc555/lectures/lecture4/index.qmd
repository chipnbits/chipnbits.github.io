...
description: >-
    Tikhonov regularization is a common technique used in inverse theory to stabilize ill-posed problems. In this lecture, we derive the Tikhonov regularization technique, and we also look at a least squares solution that does not require computing the full SVD of the matrix $A$, using the conjugate gradient method.
categories:
  - Optimization
  - Inverse Theory
  - Python
...
We can restore uniqueness by adding a regularization term that influences the selection of the minimizing $x$. The Tikhonov regularization technique adds a penalty term to the least squares problem:
...
$$ \min \frac{1}{2} x^T A^T A x - b^T A x + \frac{1}{2}\norm{b}^2 + \frac{1}{2} \lambda x^T L^T L x $$
$$ = \min \frac{1}{2} x^T H x - b^T A x + \frac{1}{2}\norm{b}^2$$

where $H = A^T A + \lambda L^T L$ is the Hessian matrix, which is symmetric and positive semi-definite, being a sum of Gram matrices. If we choose an appropriate $\lambda$ and $L$, then the Hessian matrix is positive definite and the problem is well-posed. In the case where $L=I$, the Hessian becomes full rank for any $\lambda > 0$ and the problem is well-posed. The property that $H \succ 0$ guarantees that the matrix is invertible.

#### Solution

The unique solution is given by the first-order optimality condition:
...
&= \left( V \Sigma^2 V^T + \lambda V V^T \right)^{-1} V \Sigma^T U^T b\\
&= V \left( \Sigma^2 + \lambda I \right)^{-1} \Sigma^T U^T b\\
&= V \Diag \left( \frac{\sigma_i}{\sigma_i^2 + \lambda} \right) U^T b\\
&= \sum_{i=1}^{n} \frac{\sigma_i}{\sigma_i^2 + \lambda} \ip{u_i, b} v_i
\end{align}
$$

This form is more readily comparable to some of the other methods that we have seen so far, which are presented in the table below:

## Comparison of Least Squares Methods

| Method | Solution | Notes |
| --- | --- | --- |
| Tikhonov | $x_{\text{RLS}} = \sum_{i=1}^{n} \frac{\sigma_i}{\sigma_i^2 + \lambda} \ip{u_i, b} v_i$ | $L=I$ |
| Thresholded SVD | $x_{\text{TSVD}} = \sum_{i=1}^{n} h(\sigma_i) \ip{u_i, b} v_i$ | $h(\sigma_i) = \sigma_i^{-1}$ above threshold, else $0$ |
| Gradient Flow | $x_{\text{GF}} = \sum_{i=1}^{n} \frac{1 - \exp(-\sigma_i^2 t)}{\sigma_i} \ip{u_i, b} v_i$ | early stopping at time $t$ |
...
A detailed explanation of this method can be found on [Wikipedia](https://en.wikipedia.org/wiki/Conjugate_gradient_method).
...
This is a generalization of the concept of orthogonality, with the inner product weighted by the matrix $A$ (assumed symmetric positive definite).

**Standard Orthogonality:** When $A = I$ (the identity matrix), the definition reduces to the standard concept of orthogonality. For a symmetric $A$ we also have an orthogonal decomposition into eigenvectors by the spectral theorem.

---

Returning to the problem of solving the linear system, we can express the solution $x$ as a linear combination of conjugate vectors:
...
To recover the coefficients $\alpha_i$, we can use a projection in the $A$-weighted inner product space, where conjugacy eliminates all of the cross terms:

$$ \begin{align}
A x_0 + \sum_{j=1}^n \alpha_j A p_j &= b\\
r &= b - A x_0\\
\sum_{j=1}^n \alpha_j A p_j &= r\\
\ip{p_i, \sum_{j=1}^n \alpha_j A p_j} &= \ip{p_i, r}\\
...
The step size can be seen more clearly if we trace through the minimization problem over $\alpha$ for a fixed $x$ and search direction $p$. Minimizing the least squares objective directly, with the residual defined here as $r = Ax - b$:
...
But we can also trace this through using the quadratic form for a symmetric positive definite system $Ax = b$, substituting $\tilde x = x + \alpha p$ into $\frac{1}{2} \tilde x^T A \tilde x - \tilde x^T b$:

$$
\begin{align}
& \min_\alpha \frac{1}{2} \tilde x^T A \tilde x - \tilde x^T b  \\
&= \frac{1}{2} \left( x^T A x + 2 \alpha p^T A x + \alpha^2 p^T A p \right) - x^T b - \alpha p^T b\\
0&= p^T A x + \alpha p^T A p - p^T b \\
\alpha &= \frac{p^T (b - Ax)}{p^T A p} = \frac{\ip{p, r}}{\ip{p, A p}}
\end {align}
$$

where in this second form $r = b - Ax$, recovering the step size used in the algorithm above.
