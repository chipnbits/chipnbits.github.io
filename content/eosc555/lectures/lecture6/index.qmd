...
Returning to the Lotka-Volterra model, we can now use automatic differentiation to compute the Jacobian matrix of the forward model. In fact, it can be shown that we can perform Gauss-Newton optimization more efficiently by using the Jacobian-vector product (JVP) and the vector-Jacobian product (VJP) instead of the full Jacobian matrix, since what the algorithm truly requires is the product of the Jacobian (or its transpose) with a vector. This equates to a directional derivative.
...
If we make $\epsilon$ very small, then the Jacobian-vector product of the forward problem can be numerically approximated with an error bounded by a small $\mathcal{O}(\epsilon)$ term. The next step to fully recover the Jacobian is to take the derivative with respect to $v$ of the left-hand side of the equation:

$$ \nabla_v \frac{d}{d \epsilon} F(p_k + \epsilon v) = J_k$$

The derivative with respect to $v$ can be traced through with automatic differentiation. So we apply a chain of operations — the PyTorch Jacobian-vector product, followed by backpropagation onto the surrogate $v$ that was passed to the function — to get the Jacobian of the forward model. The same principles can be used to recover $J_k^T$.

There is also a direct method available for computing the Jacobian matrix using the torch library. Both cases are shown below. Note that the tensors have the `requires_grad=True` flag set to allow gradients to be computed; it indicates that the tensor is part of the computational graph for backpropagation, tracing how much each element of $v$ contributed to the `jvp` result.

The fundamental use of `jvp` and `vjp` is to compute the action of the Jacobian or its transpose on a vector without forming the full matrix — that is, the directional derivative $J_k v$ or the transposed product
...
Now, all the previous theory can be combined to form a PyTorch training loop that solves the non-linear least squares problem using the Gauss-Newton method, utilizing the conjugate gradient method to solve the normal equations involved. The model will be fit exclusively to the prey population of the Lotka-Volterra system. This simulates a scenario where the predator population is not observed and may be difficult to measure, but more reliable measurements are available for the prey population.
...
To take the model a step further, it can be used to generate a toy dataset for fitting the model parameters with the Gauss-Newton optimization method. To make a dataset that cannot be fit perfectly, time-varying parameters with perturbation terms are used to produce an interesting dataset. We define a function that can generate multiple realizations of the Lotka-Volterra model with perturbations, then select the first realization to plot the time series and phase space of the model.
...
As can be seen from the data, the perturbations over time make the dynamics of the system only roughly periodic. This will make the optimization problem more interesting to solve.

### Jacobian Vector Product and Directional Derivatives

Now is a good time to code and check a working system for taking the Jacobian-vector products required by the Gauss-Newton method, using the functions defined earlier. We will assume that we have the prey data and that we are trying to recover the model parameters. To check the correctness of the code, we can compare the results of the `jvp` and `vjp` functions using the adjoint identity. The value $\langle w, J_k v \rangle$ is a scalar, so it should equal its transpose:
$$ \langle w, J_k v \rangle = \langle v, J_k^T w  \rangle$$

One other thing to note is that both the `jvp` and `vjp` functions also output the value of the function evaluated at the point that is passed to them, about which the Jacobian is computed. So we get both $F(p)$ and $J_k v$ from the `jvp` function, and a similar pair from the `vjp` function.
...
To implement this in code, we will also need a conjugate gradient solver for the problem
$$ J_G(p_k)^T J_G(p_k)s_k = J_G(p_k)^T r_k$$

keeping in mind that we want to avoid explicit computation of the entire Jacobian when the goal is only to apply it to a vector. To do this, the $J_G(p_k)^T J_G(p_k)$ operator can be coded as a single function `Hmv` that takes a vector and returns the product of the Hessian approximation with that vector. We then use this function in a standard implementation of the conjugate gradient method. The conjugate gradient method below has been set up to accept a callable function $A$ that acts like the matrix operator $A$, bypassing the need to form the full matrix, since we only ever use it through products.
...
\For{$k = 0, 1, 2, \ldots$}
    ...
    \State Compute the residual $r_k = -G(p_k) = d - QF(p_k)$ (forward model)
...
A run of the Gauss-Newton optimization method can be performed on the Lotka-Volterra model to fit the prey population data. The optimization method will be run for a maximum of $45$ iterations, with a tolerance that exits early if the step size becomes small enough, indicating a local minimum. The results of the optimization can be plotted against the true data, both prey and predator, to see how well the optimization method has recovered the unobserved predator population.
...
# Make a final plot of both the true predator-prey data and the predicted data
...
The solver also stores the forward model output from each successive iteration in the predictions list. The optimization process can be animated from these successive predictions to get a visual understanding of the optimization method.
...
The range of possible solutions and the dimensionality of the problem expand from $4$ parameters to $4 \times nt$ parameters, which means there are more parameters than actual data points. As a result, any dataset could be fit perfectly — but the fit might not be the correct one. This issue is a hallmark of ill-posed inverse problems: the optimization algorithm will still converge to a solution, but it may not be the true one.

Since the ground truth of both predator and prey populations is known, the optimization algorithm can be run with time-dependent parameters, which allows for more overfitting. Holding the parameters fixed in time is a form of regularization applied to the problem, and removing it will change the results of the optimization.
...
The Gauss-Newton optimization method is a powerful tool for solving non-linear least squares problems in a fast and efficient manner. It can be extended to any problem formulated as a vector of residuals — generally, any $\| G(p) \|^2$ to be optimized over $p$. Efficiency is improved by using the Jacobian-vector product to bypass the costly computation of a full Jacobian when all that is required is a directional derivative. The normal equations also present a sub-problem within the optimization routine, which can be solved with the conjugate gradient method to find the step $s_k$. This step direction is then used to perform the parameter update in the outer optimization algorithm. Increasing the complexity of the problem by allowing time-varying parameters can lead to overfitting and ill-posedness, but the optimization algorithm will still converge to a solution.
