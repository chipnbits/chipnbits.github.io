...
$$ A = \begin{bmatrix}  \vec{a}_1 \\ \vec{a}_2 \\ a_3 \end{bmatrix} \quad \vec{x} = \begin{bmatrix} b_1 \\ b_2 \\ b_3 \end{bmatrix} $$

In this case we have an *overdetermined* system with three eqautions, two unknowns, and three data samples. If the system of equations is full rank then we are trying to map from a 2D space to a 3D space: $A: \mathbb{R}^2 \rightarrow \mathbb{R}^3$. In this case there is no exact solution to the system for any $b$ that is not in the column space of $A$.
