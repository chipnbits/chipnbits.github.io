...
Often we wish to find the gradient of a multivariable function that is formulated as a linear algebra operation. In this case there are some useful "vector" derivatives and rules that can simplify the process of calculating more complex expressions. The gradient with respect to a vector $\mathbf{x}$ is generally denoted $\nabla_{\mathbf{x}}$, or alternatively $\partial_{\mathbf{x}}$ — somewhat of an abuse of notation.

#### 1.  A Warmup

$$\phi(x) = a^\top x = \sum_i a_i x_i$$

This is a vector dot product, and the gradient is simply the vector $a$. There is a subtlety here in that the gradient is usually shaped as a column vector, but this is not always the case: some authors, particularly in statistics, prefer the row-vector convention, which can cause confusion. The general convention is a column vector.
...
Based on the previous example we might expect to get $A^\top$, however no transpose occurs in this case. Here $\phi$ is vector-valued, so the appropriate derivative is the Jacobian rather than a gradient, and each row of the Jacobian is the gradient of one output component — giving back $A$ itself.
...
One way to determine the gradient is to expand the expression and evaluate a single $\frac{\partial}{\partial x_i}$ term; this method can be found in [Mark Schmidt's notes](https://www.cs.ubc.ca/~schmidtm/Courses/340-F16/linearQuadraticGradients.pdf). Instead, we can apply a product rule for vector differentiation:

$$\frac{d f(g,h)}{d x} = \frac{d (g(x)^\top)}{d x} \frac{\partial f(g,h)}{\partial g} + \frac{d (h(x)^\top)}{d x} \frac{\partial f(g,h)}{\partial h}$$

Writing $\phi(x) = g^\top h$ with $g = x$ and $h = Ax$, we have $\frac{\partial \phi}{\partial g} = h$, $\frac{\partial \phi}{\partial h} = g$, $\frac{d (g^\top)}{dx} = I$, and $\frac{d (h^\top)}{dx} = A^\top$. Then:

$$ \begin {align*}
\phi(x) &= x^\top A x \\
\nabla_{\mathbf{x}} \phi &= \frac{d (x^\top)}{dx} (Ax) + \frac{d ((Ax)^\top)}{dx} \, x\\
&= I (A x) + A^\top x\\
&= (A + A^\top) x
\end {align*}
$$

This fits with the scalar quadratic form, where we end up with $(cx^2)' = (c + c^\top)x = 2cx$ for a scalar $c$.
...
For this one, let $y = Ax$, so that each element of the vector $y$ is $y_i = \sum_j A_{ij} x_j$. The Hadamard product is a vector $z$ where $z_i = y_i^2$. Since the output is a vector, we compute the Jacobian rather than a gradient.
...
If we can recover this, then we have the derivative of the Hadamard product.

$$
\begin{align*}
z_i &= y_i^2 = \left( \sum_j A_{ij} x_j \right)^2\\
\frac{\partial}{\partial x_j} y_i^2 &= 2 y_i \frac{\partial y_i}{\partial x_j} = 2 y_i A_{ij}\\
\frac{d\vec{z}}{d\vec{x}} &= 2 \begin{bmatrix} y_1 A_{11} & y_1 A_{12} & \cdots & y_1 A_{1n} \\
y_2 A_{21} & y_2 A_{22} & \cdots & y_2 A_{2n} \\
\vdots & \vdots & \ddots & \vdots \\
y_n A_{n1} & y_n A_{n2} & \cdots & y_n A_{nn} \end{bmatrix}\\
...
At this point, the question is whether $A^\top A$ is invertible. The invertibility of $A^\top A$ is determined by the rank of $A$ — the number of independent columns. Suppose $A^\top A x = 0$ for some $x$. This requires $Ax$ to lie in the null space of $A^\top$; but $N(A^\top) = R(A)^\perp$ is orthogonal to the range, and $Ax \in R(A)$, so the only possibility is $Ax = 0$. Therefore $A^\top A x = 0$ implies $Ax = 0$, and $A^\top A$ has the same null space as $A$. If the null space of $A$ is $\{0\}$, then the null space of $A^\top A$ is also $\{0\}$, in which case $A^\top A$ is symmetric positive definite and invertible.

In summary, $A^\top A$ is invertible $\iff$ $A$ has full column rank, that is, all of its columns are independent. For a non-square $m \times n$ matrix, a wide matrix ($m < n$, more columns than rows) can never satisfy this condition. A tall matrix ($m > n$) satisfies the condition if its columns are independent.
...
where $\alpha$ is the learning rate, which can depend on the problem and the gradient. Substituting the gradient of the least squares problem, we have:
...
This ODE is the continuous version of the gradient descent algorithm, also known as the *gradient flow*. Since this is a linear first-order ODE, we can solve it analytically. The general method for a linear ODE system is to find the homogeneous solution and a particular solution. For the homogeneous equation we guess $x_h = v e^{\lambda t}$:

$$ \begin{align}
x' + A^\top A x &= A^\top b\\
x_h' + A^\top A x_h &= 0 \qquad \text{Homogeneous}\\
\lambda v e^{\lambda t} + A^\top A v e^{\lambda t} &= 0\\
(\lambda I + A^\top A) v &= 0\\
\lambda &= -\mu_i, \quad v = \text{eigenvectors of } A^\top A
\end{align}
$$

where $\mu_i \geq 0$ are the eigenvalues of $A^\top A$. Before continuing further along this line, we can see that the solutions will be closely related to the SVD, because it contains the information on these eigenvalues and eigenvectors. So we can solve the ODE with the SVD.
...
At this stage, since everything has been diagonalized, all of the equations are decoupled and independent, so we can solve each component separately. Writing $\lambda_i$ for the singular values on the diagonal of $\Sigma$ (so that $\Sigma^2$ has entries $\lambda_i^2$), we find the homogeneous $z_h$ and particular $z_p$ solutions for each component:
...
Then, putting all of the equations back together, we have

$$z = \text{diag}\left( \lambda_i^{-1} (1 - \exp (-\lambda_i^2 t)) \right) \hat b$$

Substituting back in for $x$ and $b$, we get:

$$x = V \, \text{diag}\left( \lambda_i^{-1} (1 - \exp (-\lambda_i^2 t)) \right) U^\top b$$

If we stare at this long enough, it begins to look a lot like the pseudoinverse solution from earlier, $x = V \Sigma^{-1} U^\top b$, except that in this case there is a time dependence. In the limit as $t \rightarrow \infty$, the exponential term goes to zero and we are left with the pseudoinverse solution. This is a nice way to see that the pseudoinverse is the limit of the gradient descent algorithm. What we may be more interested in is what happens at earlier stages, since each decay term depends on its corresponding singular value.

For a simple matrix problem, we can create a matrix and plot the time evolution of the diagonal entries of interest. In a sense, we have inverse singular values that evolve in time at different rates.
...
So we can use early stopping to prevent the flow from reaching the optimal point — a very useful technique. When it comes to inverse theory, we are often not interested in the exact least squares solution, but rather in getting somewhere close that is not too noisy. This method differs from the thresholded pseudoinverse of the previous lecture in that it allows some blending of the smaller singular values, while their propensity for blowing up is controlled by the exponential decay term and early stopping.

### Example for Image Recovery using Analytic Solution

Let us refer back to the problem of estimating the original image from a blurred and noisy observation. We can monitor the time evolution of the estimate using gradient flow. The code below defines the problem again, recovering the SVD for the $32 \times 32$ image, which will be used to solve the ODE for the gradient flow.
...
Now that we have the matrix form of the forward operator `Amat` defined, along with the forward result `b` and the decomposition `U, S, V`, we can run the pseudoinverse gradient flow method as before. In this case we will be computing:

$$ x = V \, \text{diag}\left( \lambda_i^{-1} (1 - \exp (-\lambda_i^2 t)) \right) U^\top b$$

Since this represents an evolution over time, an animation can be created to show the time evolution of the image recovery, along with the effect of continuing into the regime where amplified noise dominates.

Recall the original and distorted images, with a small amount of noise $\epsilon$ added:
...
We also saved the best stopping time discovered during the recovery (using prior knowledge of the ground truth), so we can inspect that image — the best reconstruction achievable with the gradient flow method.
...
In this case we were able to compute the matrix form of $A$ explicitly and take its SVD, but in many cases this is too expensive, or there is no closed-form analytic solution for the early stopping technique. In such cases we still wish to apply the adjoint. The question then is how to recover the adjoint operator from the `Amv` operator. Automatic differentiation provides a helpful tool: by tracking the gradients of the forward operator, we can recover the action of the adjoint in a very general way.
...
We can compute the adjoint explicitly for the `Amv` operator based on its computation from earlier. The unitary discrete Fourier transform matrix $F$ has the property that $F^{-1} = F^*$, its conjugate transpose, so we can use this to compute the adjoint.

$$
\begin{align}
A(x) &= \mathcal{F}^{-1} \left( \mathcal{F}(P) \odot \mathcal{F}(x) \right)\\
&= F^{*} \, \text{diag} (F(P)) \, F x\\
A^\top(v) &= F^{*} \, \text{diag} (F(P))^* \, F v\\
\end{align}
$$

Here the Hadamard product of the two vectors has been recast in matrix form by diagonalizing the vector $F(P)$, the Fourier transform of the point spread function. From this form, the adjoint of the operator is obtained by taking the conjugate transpose of the entire operation; since $A$ is a real operator, its adjoint coincides with its transpose, and only the diagonal factor picks up a complex conjugate.
...
The difference is within the bounds of numerical precision, and the code appears to be working correctly.

## Gradient Descent with Adjoint

We can now use the operators (functions) defined earlier to set up a simple gradient descent algorithm with a step size and early stopping. This produces a recovered image while bypassing the SVD computation, which may be very expensive for large matrices.
...
Note that PyTorch does have the machinery to run autograd on the least squares objective itself, but in this general method we use the adjoint to compute the gradient (invoking autograd only indirectly). This framework is the most general for cases where there is no explicit analytic solution to the least squares problem, but we have access to the forward operator and its adjoint.
